...
class AdvancedSearch(HighlightedSearchForm):
    ...
    def search(self):
        ...
        if self.cleaned_data['maintainers']:
            sqs = sqs.filter_or(maintainers__search=self.cleaned_data['q'])

        if self.cleaned_data['name']:
            sqs = sqs.filter_or(name__search=self.cleaned_data['q'])

        if self.cleaned_data['variants']:
            sqs = sqs.filter_or(variants__search=self.cleaned_data['q'])
